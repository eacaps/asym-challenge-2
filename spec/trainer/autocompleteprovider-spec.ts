...
import AutocompleteProvider from "../../src/trainer/autocompleteprovider";
...
describe("trainer", () => {
  ...
  it("tests sample", () => {
    ...
    expect(top_thi_result.word).toMatch("thing");
    expect(top_thi_result.confidence).toBe(2);
    expect(StringifiedContains({ word: "think", confidence: 1 }, thi_result));
    expect(StringifiedContains({ word: "third", confidence: 1 }, thi_result));
    expect(StringifiedContains({ word: "this", confidence: 1 }, thi_result));
    ...
    expect(top_nee_result.word).toMatch("need");
    expect(top_nee_result.confidence).toBe(1);
    ...
    expect(StringifiedContains({ word: "that", confidence: 2 }, th_result));
    expect(StringifiedContains({ word: "thing", confidence: 2 }, th_result));
    expect(StringifiedContains({ word: "think", confidence: 1 }, th_result));
    expect(StringifiedContains({ word: "this", confidence: 1 }, th_result));
    expect(StringifiedContains({ word: "third", confidence: 1 }, th_result));
    expect(StringifiedContains({ word: "the", confidence: 1 }, th_result));
    expect(
      StringifiedContains({ word: "thoroughly", confidence: 1 }, th_result)
    );
  });
});
